...
def system_health_check_task() -> Dict[str, Any]:
    ...
    try:
        ...
        # Проверка процессов
        process_check = _check_processes()
        health_status["checks"]["processes"] = process_check
        
        # Проверка директорий
        directories_check = _check_storage_directories()
        health_status["checks"]["directories"] = directories_check
        
        # Проверка подключения к IMAP
        imap_check = _check_imap_connection()
        health_status["checks"]["imap"] = imap_check
...
def _check_processes() -> Dict[str, Any]:
    ...
    try:
        # Проверяем основные процессы системы
        ...
        return {
            ...
            "processes": running_processes[:5],  # Показываем только первые 5
...
def _check_storage_directories() -> Dict[str, Any]:
    ...
    try:
        ...
        # Все директории должны существовать и быть записываемыми
        all_ok = all(
            info["exists"] and info["writable"] 
            for info in directory_status.values()
        )
...
def _check_imap_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        # Создаем клиент и пробуем подключиться
...
def generate_daily_metrics_task() -> Dict[str, Any]:
    """
    Генерация ежедневных метрик системы
    """
    try:
        ...
        files_processed = len([
            f for f in safe_storage_dir.glob('**/*') 
            if f.is_file() and 
            datetime.fromtimestamp(f.stat().st_mtime) >= start_date
        ])
        
        quarantine_dir = Path(settings.quarantine_dir)
        files_quarantined = len([
            f for f in quarantine_dir.glob('**/*') 
            if f.is_file() and 
            datetime.fromtimestamp(f.stat().st_mtime) >= start_date
        ])
        ...
        metrics["metrics"] = {
            ...
            "processing_rate": round(files_processed / 24, 2),  # файлов в час
            "quarantine_rate": round((files_quarantined / (files_processed + files_quarantined)) * 100, 2) if (files_processed + files_quarantined) > 0 else 0
        }
...
def alert_on_errors_task(threshold: int = 5) -> Dict[str, Any]:
    """
    Проверка ошибок и отправка уведомлений при превышении порога
    
    Args:
        threshold: Порог количества ошибок для отправки уведомления
    """
    try:
        # Простая проверка логов на ошибки за последний час
        ...
        if error_count >= threshold:
            # Здесь должна быть отправка уведомлений (email, Slack, etc.)
